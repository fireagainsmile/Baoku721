...
// File: @openzeppelin/contracts/security/pausable.so
...
contract BaokuNFT is ERC721, Ownable{
    ...
    // @dev mapping address to all the properties
    mapping(address => mapping(uint256 => uint256)) private _properties;
    // @dev mapping token 2 total issued amount
    mapping(uint256 => uint256) private _totalIssuedAmount;
    // @dev mapping token current edition number counter
    ...
    * @dev See {IERC721Metadata-tokenURI}.
    ...
    function tokenMeta(uint256 _tokenId) external view returns (string memory, string memory,string memory,string memory){
    ...
    // @dev get how many editions in specified token id of an account
    function editionBalance(address _account, uint256 _tokenId) external view returns(uint256) {
        return _balances[_account][_tokenId];
    }

    // @dev  increment the edition number by token ID with amount
    function increment(uint256 _tokenId, uint256 _amount) internal {
    unchecked {
        _tokenCurrentNumber[_tokenId] += _amount;
    }
    }
    ...
    function safeMintBatchEdition(uint256 _tokenId, address[] memory _accounts) internal{
        ...
        for (uint256 i = 0; i < _accounts.length; i++){
            _editionOwner[_tokenId][_currentId + i] = _accounts[i];
            _balances[_accounts[i]][_tokenId] += 1;
        }
        increment(_tokenId, _accounts.length);
        emit TransferredBatchEdition(_tokenId, _msgSender(), _accounts);
    }

    // @dev transfer specified edition id to another user
    function safeTransferEdition(uint256 _tokenId,uint256 _editionId, address _to) internal{
        require(_to != address (0), "Edition controller: transfer to zero address");
        address _from = _msgSender();
        require(_from == _editionOwner[_tokenId][_editionId], "Edition controller: you are not the owner");
    ...
    function claimSingleEdition(uint256 _tokenId, address _to) external {
        require(_exists(_tokenId), "BaoKuNFT: token does not exist");
        require(_msgSender() == ownerOf(_tokenId),"BaoKuNFT: not the token ow");
        safeMintEdition(_tokenId, _to);
    }

    function claimBatchEdition(uint256 _tokenId, address[] memory _accounts) external {
        require(_exists(_tokenId), "BaoKuNFT: token does not exist");
        require(_msgSender() == ownerOf(_tokenId),"BaoKuNFT: not the token ow");
